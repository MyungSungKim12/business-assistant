...
현재는 고객·거래처, 일정·할 일, 문서 자동화, 매출·지출, 파일·자료의 서버 API가 구현되어 있습니다. 가져오기·내보내기, 보고서·분석, 자동화, 알림 등은 다음 범위입니다.
...
- `finance_transactions`: 조직별 수입·지출 거래 원장
...
설정되면 인증·조직·구독 권한 및 CRM 고객·문서·매출지출 API를 함께 확인할 수 있습니다.
업무 데이터는 조직 멤버십, 기능 권한, 그리고 Supabase RLS를 함께 검사합니다. 파일 첨부,
가져오기·내보내기, 보고서·분석 등 나머지 범용 업무 기능은 순서대로 구현합니다.
